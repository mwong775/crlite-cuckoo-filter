#ifndef CUCKOOHASH_HASHTABLE_H_
#define CUCKOOHASH_HASHTABLE_H_
...
namespace cuckoohash {
     ...
     * KeyEqual - type of equality comparison func
    ...
    class hashtable {
        ...
        public:
            ...
            /**
             * Table Parameters: The number of slots per hash bucket
             */
            static constexpr uint16_t slot_per_bucket() { return SLOT_PER_BUCKET; }

            /**
             * Contructors, Destructors, and Assignment
             * Creates new hashset instace
             * 
             * n - number of elements to reserve space for initially
             * hf - hash function instance to use
             * equal - equality function to use
             * alloc - allocator instance to use
             */

            hashtable(size_type n = DEFAULT_SIZE, const HASH &HF = Hash(),
            ...
            /**
         * Constructs the table with the contents of the range @c [first, last].  If
         ...
         * unspecified.
         *
         * @param other the table being copied
         */
         ...
         *
         * @param other the table being copied
         * @param alloc the allocator instance to use with the table
         */
        ...
        /**
         * Move constructor with separate allocator. If the table being moved is being
        ...
        /**
         * Constructs the table with the contents of initializer list @c init.
         *
         * @param init initializer list to initialize the elements of the table with
         * @param n the number of elements to reserve space for initially
         * @param hf hash function instance to use
         * @param equal equality function instance to use
         * @param alloc allocator instance to use
         */
        hashtable(std::initializer_list<key_type> init,
                        size_type n = DEFAULT_SIZE, const Hash &hf = Hash(),
                        const KeyEqual &equal = KeyEqual(),
                        const Allocator &alloc = Allocator())
            : hashtable(init.begin(), init.end(), n, hf, equal, alloc) {}

        /**
         * Exchanges the contents of the table with those of @p other
         *
         * @param other the table to exchange contents with
         */
         ...
         *
         * @param other the table to assign from
         * @return @c *this
         */
        ...
        hashtable &operator=(std::initializer_list<value_type> ilist) {
            ...
            insert(item); // item.first, item.second
            }
            return *this;
        }
        ...
        /**
         * Returns the allocator associated with the table
         *
        ...
        template <typename K, typename F> bool find_fn(const K &key, F fn) const {
            ...
            if (pos.status == ok) {
            fn(buckets_[pos.index].key(pos.slot));
            return true;
            } else {
            return false;
            }
        ...
        template <typename K, typename F> bool erase_fn(const K &key, F fn) {
            ...
            const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2);
            if (pos.status == ok) {
            if (fn(buckets_[pos.index].key(pos.slot))) {
         ...
         */
        template <typename K> bool find(const K &key, key_type &keyval) const {
            return find_fn(key, [&keyval](const key_type &k) mutable { keyval = k; });
        }
         ...
         */
        template <typename K> key_type find(const K &key) const {
            const hash_value hv = hashed_key(key);
            const auto b = snapshot_and_lock_two<normal_mode>(hv);
            const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2);
            if (pos.status == ok) {
        ...
        template <typename K> bool contains(const K &key) const {
            return find_fn(key, [](const key_type &) {});
        }
        ...
        // will NOT be using - need new update strategy
        ...
        template <typename K, typename... Args> bool insert(K &&key, Args &&... val) {
            return upsert(std::forward<K>(key), [](key_type &) {});
        }
        ...
        // probably also not used - stick to insert only
        ...
        template <typename K> bool erase(const K &key) {
            return erase_fn(key, [](key_type &) { return true; });
        }
        ...
        void add_locks_from_other(const hashtable &other) {
        ...
        struct AllUnlocker {
            void operator()(hashtable *table) const {
            for (auto it = first_locked; it != table->all_locks_.end(); ++it) {
                locks_t &locks = *it;
        ...
        using AllLocksManager = std::unique_ptr<hashtable, AllUnlocker>;
        ...
        void add_to_bucket(const size_type bucket_ind, const size_type slot,
                            const partial_t partial, K &&key) {
            buckets_.setKey(bucket_ind, slot, partial, std::forward<K>(key));
        ...
        bool cuckoopath_move(const size_type hp, CuckooRecords &cuckoo_path,
            ...
            buckets_.setKey(to.bucket, ts, fb.partial(fs), fb.movable_key(fs),
                            std::move(fb.key(fs)));
            buckets_.eraseKey(from.bucket, fs);
            if (depth == 1) {
        ...
        void move_bucket(buckets_t &old_buckets, buckets_t &new_buckets,
            ...
            new_buckets.setKey(dst_bucket_ind, dst_bucket_slot++,
                                old_bucket.partial(old_bucket_slot),
                                old_bucket.movable_key(old_bucket_slot),
                                std::move(old_bucket.key(old_bucket_slot)));
            }
        }
        ...
        cuckoo_status cuckoo_expand_simple(size_type new_hp) {
            ...
            // from buckets_ and old_buckets_. Allow this table to spawn extra threads if
            // it needs to resize during the resize.
            hashtable new_map(hashsize(new_hp) * slot_per_bucket(),
                                hash_function(), key_eq(), get_allocator());
            new_table.max_num_worker_threads(max_num_worker_threads());
            ...
            parallel_exec(
                ...
                try {
                    ...
                    for (size_type j = 0; j < slot_per_bucket(); ++j) {
                        ...
                        new_table.insert(bucket.movable_key(j),
                                        std::move(bucket.key(j)));
                        }
                    }
                    }
                } catch (...) {
            ...
            maybe_resize_locks(new_table.bucket_count());
            buckets_.swap(new_table.buckets_);
        ...
        public:
        /**
        * An ownership wrapper around a @ref hashtable table instance. When
        ...
        */
        ...
        public:
            ...
            */
            ...
            return hashtable::slot_per_bucket();
            }
            ...
            * to their @ref hashtable counterparts. Only new functions or
            ...
            */
            ...
            hasher hash_function() const { return table_.get().hash_function(); }

            key_equal key_eq() const { return table_.get().key_eq(); }

            allocator_type get_allocator() const { return table_.get().get_allocator(); }

            size_type hashpower() const { return table_.get().hashpower(); }

            size_type bucket_count() const { return table_.get().bucket_count(); }

            bool empty() const { return table_.get().empty(); }

            size_type size() const { return table_.get().size(); }

            size_type capacity() const { return table_.get().capacity(); }

            double load_factor() const { return table_.get().load_factor(); }

            void minimum_load_factor(const double mlf) {
            table_.get().minimum_load_factor(mlf);
            }

            double minimum_load_factor() const {
            return table_.get().minimum_load_factor();
            }

            void maximum_hashpower(size_type mhp) { table_.get().maximum_hashpower(mhp); }

            size_type maximum_hashpower() const {
            return table_.get().maximum_hashpower();
            }

            void max_num_worker_threads(size_type extra_threads) {
            table_.get().max_num_worker_threads(extra_threads);
            }

            size_type max_num_worker_threads() const {
            return table_.get().max_num_worker_threads();
            }
            ...
            */
            ...
            iterator begin() { return iterator(table_.get().buckets_, 0, 0); }

            const_iterator begin() const {
            return const_iterator(table_.get().buckets_, 0, 0);
            }
            ...
            */
            ...
            iterator end() {
            const auto end_pos = const_iterator::end_pos(table_.get().buckets_);
            return iterator(table_.get().buckets_,
            ...
            const auto end_pos = const_iterator::end_pos(table_.get().buckets_);
            return const_iterator(table_.get().buckets_,
                                    static_cast<size_type>(end_pos.first),
                                    static_cast<size_type>(end_pos.second));
            }
            ...
            void clear() { table_.get().cuckoo_clear(); }
            ...
            */
            ...
            hash_value hv = table_.get().hashed_key(key);
            auto b = table_.get().template snapshot_and_lock_two<locked_table_mode>(hv);
            table_position pos =
                table_.get().template cuckoo_insert_loop<locked_table_mode>(hv, b, key);
            if (pos.status == ok) {
                table_.get().add_to_bucket(pos.index, pos.slot, hv.partial,
                                        std::forward<K>(key));
            } else {
                assert(pos.status == failure_key_duplicated);
            }
            return std::make_pair(iterator(table_.get().buckets_, pos.index, pos.slot),
                                    pos.status == ok);
            }

            iterator erase(const_iterator pos) {
            table_.get().del_from_bucket(pos.index_, pos.slot_);
            return iterator(table_.get().buckets_, pos.index_, pos.slot_);
            }
            ...
            const hash_value hv = table_.get().hashed_key(key);
            const auto b =
                table_.get().template snapshot_and_lock_two<locked_table_mode>(hv);
            const table_position pos =
                table_.get().cuckoo_find(key, hv.partial, b.i1, b.i2);
            if (pos.status == ok) {
                table_.get().del_from_bucket(pos.index, pos.slot);
                return 1;
            } else {
                return 0;
            }
            ...
            const table_position pos =
                table_.get().cuckoo_find(key, hv.partial, b.i1, b.i2);
            if (pos.status == ok) {
                return iterator(table_.get().buckets_, pos.index, pos.slot);
            } else {
                return end();
            }
            }
            ...
            const table_position pos =
                table_.get().cuckoo_find(key, hv.partial, b.i1, b.i2);
            if (pos.status == ok) {
                return const_iterator(table_.get().buckets_, pos.index, pos.slot);
            } else {
                return end();
            }
            }

            template <typename K> key_type &at(const K &key) {
            auto it = find(key);
            if (it == end()) {
                throw std::out_of_range("key not found in table");
            } else {
            ...
            template <typename K> const key_type &at(const K &key) const {
            auto it = find(key);
            if (it == end()) {
                throw std::out_of_range("key not found in table");
            } else {
            ...
            * hashtable::insert, except that the value is default-constructed,
            ...
            */
            ...
            const auto b =
                table_.get().template snapshot_and_lock_two<locked_table_mode>(hv);
            return table_.get().cuckoo_find(key, hv.partial, b.i1, b.i2).status == ok ? 1 : 0;
            }
            ...
            /**
            * This has the same behavior as @ref hashtable::rehash, except
            * that we don't return anything.
            */
            void rehash(size_type n) {
            table_.get().template cuckoo_rehash<locked_table_mode>(n);
            }

            /**
            * This has the same behavior as @ref hashtable::reserve, except
            * that we don't return anything.
            */
            void reserve(size_type n) {
            table_.get().template cuckoo_reserve<locked_table_mode>(n);
            }
        ...
        private:
            ...
            // (but expose it to the hashtable class), since we don't want users
            // calling it. We also complete any remaining rehashing in the table, so
            // that everything is in table.buckets_.
            locked_table(hashtable &table) noexcept
                : table_(table),
                all_locks_manager_(table.lock_all(normal_mode())) {
            table.rehash_with_workers();
            }

            // Dispatchers for methods on hashtable

            buckets_t &buckets() { return table_.get().buckets_; }

            const buckets_t &buckets() const { return table_.get().buckets_; }

            void maybe_resize_locks(size_type new_bucket_count) {
            table_.get().maybe_resize_locks(new_bucket_count);
            }

            locks_t &get_current_locks() { return table_.get().get_current_locks(); }

            // A reference to the table owned by the table
            std::reference_wrapper<hashtable> table_;
        ...
        /**
        * Specializes the @c std::swap algorithm for @c hashtable. Calls @c
        * lhs.swap(rhs).
        *
        * @param lhs the table on the left side to swap
        * @param lhs the table on the right side to swap
        */
        template <class Key, class T, class Hash, class KeyEqual, class Allocator,
                std::size_t SLOT_PER_BUCKET>
        void swap(
            hashtable<Key, Hash, KeyEqual, Allocator, SLOT_PER_BUCKET> &lhs,
            hashtable<Key, Hash, KeyEqual, Allocator, SLOT_PER_BUCKET>
                &rhs) noexcept {
        lhs.swap(rhs);
